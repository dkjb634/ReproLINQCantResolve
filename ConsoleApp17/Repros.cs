...
public class Repros
{
    public List<ReturnTbl_Master_Board> Repro1(Tbl_Sleeve_Board param)
    {
        using (MyDBContext _context = new MyDBContext())
        {
            ...
            var rlst =
                (from x in key
                    ...
                    join st in sl on Security.EncryptSHA512(x.UsedCode + x.Idx_data) equals st.Encode_Board into sj
                    ...
                    select new ReturnTbl_Master_Board
                    {
                        ...
                        Photo = "photoUrl",
                        IsDeletable = string.IsNullOrEmpty(x.UsedCode)
                            ? false
                            : c2.UsedCode.Equals(long.Parse(param.MemberUsedCode)),
                        IsMemberInComments = c1 == null
                            ? 0
                            : (_context.Tbl_Sleeve_Board.Where(x => x.Datt_Depth >= 1 &&
                                                                    x.Encode_Board.Contains(c1.Encode_Board)
                                                                    && x.UsedCode == param.MemberUsedCode).ToList()
                                .Count()),
                        IntReproField = _context.Tbl_Sleeve_Board.Where(x => x.Datt_Depth >= 1 &&
                                                                             x.Encode_Board.Contains(c1.Encode_Board) &&
                                                                             x.UsedCode == param.UsedCode).ToList()
                            .Count()
    ...
    public List<ReturnTbl_Member> Repro2()
    {
        using (MyDBContext _context = new MyDBContext())
        {
            ...
            var rlst = (
                ...
                select new ReturnTbl_Member
                {
                    Mission_Status = _context.Tbl_MaS_Ongoing.Where(x => x.UsedCode == "" && x.End_Idx == 0)
                        .Count() > 0
                        ? _context.Tbl_MaS_Ongoing.Where(x => x.UsedCode == "" && x.End_Idx == 0)
                            .FirstOrDefault().Mission_Type + ":" + _context.Tbl_MaS_Ongoing
                            .Where(x => x.UsedCode == "" && x.End_Idx == 0)
                            .FirstOrDefault().Mission_Step
                        : "Finish"
                }).ToList();
